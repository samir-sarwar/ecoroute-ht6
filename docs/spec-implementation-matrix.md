...
| Carbon and impact | Ready | Electricity Maps v4 live adapter, explicit Carbon Aware fallback, fixture and last-known behavior, Redis cache/background refresh, zone/data-center lookup, processing/grid attribution evidence, same-token baseline, raw/floored deltas, unavailable-accounting exclusion, cache/router overhead, evidence/source/coefficient persistence |
